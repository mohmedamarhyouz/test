...
import { collectDeviceData, saveDeviceToDatabase } from '../services/deviceService';
...
function DeviceInfo() {
  ...
  const initializeDeviceInfo = async (forceSave = false) => {
    ...
    try {
      ...
      // allow only one auto-save per session unless forced (but still finish loading)
      if (!forceSave && savedOnceRef.current) {
        setLoading(false);
        return;
      }
